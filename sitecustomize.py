...
try:
    ...
    async def _gecko_pools(mint: str) -> list[dict]:
        try:
            ...
            for item in payload.get("data") or []:
                ...
                out.append({"chainId":"solana","pairAddress":pool_id.split("_",1)[-1],"dexId":str(attrs.get("dex_id") or attrs.get("name") or "geckoterminal"),"baseToken":{"name":attrs.get("name") or "Unknown","symbol":"?"},"priceUsd":attrs.get("base_token_price_usd"),"liquidity":{"usd":reserve},"volume":{"m5":volume.get("m5",0),"h1":volume.get("h1",0),"h6":volume.get("h6",0),"h24":volume.get("h24",0)},"txns":{},"priceChange":{},"pairCreatedAt":0})
            return out
        except Exception as exc:
    ...
    async def _trusted_price(mint: str, pairs: list[dict]) -> float | None:
        ...
        if jup and median:
            if 0.33<=jup/median<=3.0: return jup
    ...
    async def _fallback_overview(mint: str):
        ...
        if not pairs: warnings.append("No indexed AMM pool yet; token may be on a bonding curve or too new")
        if pump: warnings.append("pump.fun metadata detected")
        return security.TokenOverview(mint=mint,name=name,symbol=symbol,price_usd=price,market_cap=market_cap,liquidity_usd=sum(_f((p.get("liquidity") or {}).get("usd")) for p in pairs),change_5m=0.0,change_1h=0.0,dex=(pairs[0].get("dexId") if pairs else ("pump.fun" if pump else "-")),found=True,fdv=market_cap,volume_5m=sum(_f((p.get("volume") or {}).get("m5")) for p in pairs),volume_1h=sum(_f((p.get("volume") or {}).get("h1")) for p in pairs),volume_6h=sum(_f((p.get("volume") or {}).get("h6")) for p in pairs),volume_24h=sum(_f((p.get("volume") or {}).get("h24")) for p in pairs),pair_address=pairs[0].get("pairAddress","") if pairs else "",pool_count=len(pairs),total_supply=supply,decimals=_i(chain.get("decimals")),mint_authority=chain.get("mint_authority"),freeze_authority=chain.get("freeze_authority"),top_holder_pct=chain.get("top_holder_pct"),data_source="DexScreener + Jupiter + GeckoTerminal + Solana RPC + pump.fun",market_cap_source=("Trusted live price × on-chain supply" if supply and price else "pump.fun metadata" if pump_mcap else "Unavailable"),fetched_at_ms=int(time.time()*1000),data_quality="MEDIUM" if pairs else "LOW",data_warnings=warnings)

    async def live_overview(mint: str):
        try: security._OVERVIEW_CACHE.pop(mint,None)
        except Exception: pass
        overview=await ORIGINAL_OVERVIEW(mint)
        if not getattr(overview,"found",False): overview=await _fallback_overview(mint)
        if not getattr(overview,"found",False): return overview
        try: pairs=await security._fetch_sol_pairs(mint)
    ...
    async def _send_message_with_snapshot(self,*args,**kwargs):
        ...
        try:
            ...
            if chat_id not in settings.admin_ids or not text or "Market Cap:" not in text or "Liquidity:" not in text: return result
            match=MINT_RE.search(text)
            if not match: return result
            mint=match.group(0)
            symbol=_snapshot_symbol(text)
            overview=await live_overview(mint)
            if "🏦 Market Cap:" in text and "💧 Liquidity:" in text and getattr(overview,"found",False):
                detail=("\n\n*LIVE DATA DETAILS*"
                    f"\n• 5m volume: `${getattr(overview,'volume_5m',0):,.0f}`"
                    f"\n• 1h volume: `${getattr(overview,'volume_1h',0):,.0f}`"
                    f"\n• 24h volume: `${getattr(overview,'volume_24h',0):,.0f}`"
                    f"\n• 5m trades: `{getattr(overview,'total_trades_5m',0)}`"
                    f"\n• Buy/Sell: `{getattr(overview,'buy_sell_ratio_5m',0):.2f}`"
                    f"\n• Pools: `{getattr(overview,'pool_count',0)}`"
                    f"\n• Pool age: `{getattr(overview,'age_minutes',None) if getattr(overview,'age_minutes',None) is not None else 'unknown'}` min"
                    f"\n• Data quality: `{getattr(overview,'data_quality','LOW')}`"
                    f"\n• Price source: `{getattr(overview,'market_cap_source','provider')}`"
                    f"\n• Data source: `{getattr(overview,'data_source','unknown')}`")
                warnings=list(getattr(overview,"data_warnings",[]) or [])
                if warnings: detail += "\n• Warnings: " + " | ".join(warnings[:3])
                try: result=await result.edit_text(text+detail,reply_markup=result.reply_markup,parse_mode="Markdown")
                except Exception as exc: log.debug("Could not expand live token details: %s",type(exc).__name__)
