...
def main()->None:
    init_db()
    official_channels.ensure_official_channels_table()
    # The user handler already has a moderation-channel hook. Replace its
    # source with the persistent admin-managed implementation at startup.
    user._is_moderation_channel = official_channels.is_official_channel
    telegram_app=(TelegramApplication.builder().token(settings.bot_token).post_init(_post_init).post_shutdown(_post_shutdown).build())
    telegram_app.add_handler(MessageHandler(filters.ALL,_moderation_guard),group=-1)
    telegram_app.add_handler(CallbackQueryHandler(_moderation_guard),group=-1)
    user.register(telegram_app)
    admin.register(telegram_app)
    official_channels.register(telegram_app)
