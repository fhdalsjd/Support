"""SQLAlchemy ORM models for the Free Advertisement verification system."""
...
class ApplicationStatus(str, enum.Enum):
    PENDING_POST_CHECK = "PENDING_POST_CHECK"
...
class Application(Base):
    ...
    average_views: Mapped[float] = mapped_column(Float, default=0.0)
    ...
    referral_link_found: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    verification_deadline: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    ...
    def has_notified(self, stage: str) -> bool:
        return stage in (self.notified_stages or "").split(",")

    def mark_notified(self, stage: str) -> None:
        stages = set(filter(None, (self.notified_stages or "").split(",")))
        stages.add(stage)
        self.notified_stages = ",".join(sorted(stages))
